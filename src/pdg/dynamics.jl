...
eval(SparseConnectivityTracer.generate_code_1_to_1(:DualPhaseDescent, T_K_fwd))
...
eval(SparseConnectivityTracer.generate_code_1_to_1(:DualPhaseDescent, p_Pa_fwd))
...
eval(SparseConnectivityTracer.generate_code_1_to_1(:DualPhaseDescent, sqrt_smooth))

@register_symbolic atand_smooth(a, b)
...
calc_earth_equiv_alt(kalt) = 7963.75*(kalt/1000)/(6371 + 1.25*(kalt/1000))*1000
ref_atmo = CSV.read("src/kerbal/aero_ref.csv", DataFrame)
ref_density = ref_atmo[1, :density_kgm3]

density_data = select(ref_atmo, :z, :density_kgm3 => ByRow(r -> r/ref_density) => :r_rel)
density_lut = AeroLookup1DTs(extrapolate(interpolate((density_data[:, :z],), density_data[:, :r_rel], Gridded(Linear())), Flat()))
temp_data = select(ref_atmo, :z, :temperature)
temp_lut = AeroLookup1DTs(extrapolate(interpolate((temp_data[:, :z],), temp_data[:, :temperature], Gridded(Linear())), Flat()))
...
function make_vehicle(; 
    ...
    @parameters fin_offset[1:3] = [0, 0, 14.0], [tunable = false]
    ...
    @parameters ρω[1:2]=ones(3), [tunable = false] ρR[1:2]=ones(2), [tunable = false] ρv[1:3]=ones(3), [tunable = false] ρpos[1:3]=ones(3), [tunable = false] ρm=mdry, [tunable = false] ρτ=10.0, [tunable=false]

    @parameters ωk[1:3]=RotY(deg2rad(angle_from_pole)) * [0,0, 2π/21549.425], [tunable=false] # 2 pi radians every 21549s
    @parameters μk=3.5316e12, [tunable=false] thmax=910000, [tunable=false]
    ...
    eqns = expand_derivatives.([
        ...
        earth_equiv_alt ~ calc_earth_equiv_alt(spherical_alt)
        temp ~ temp_lut(spherical_alt)
        ...
        acc .~ (g_accel .+ (rquat(Rp) * Symbolics.scalarize(th))/(mp) .+ aero_force/(mp) .- centrifugal_accel .- coriolis_accel)
